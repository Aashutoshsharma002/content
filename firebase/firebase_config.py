...
if firebase_admin:
    # For development, we'll use just the project ID to initialize Firebase Admin
    # Note: In production, you should use a service account JSON file
    try:
        # Initialize application with project ID only
        app_options = {
            'projectId': FIREBASE_PROJECT_ID,
        }
        firebase_admin.initialize_app(options=app_options)
        logger.info(f"Firebase Admin SDK initialized successfully with project ID: {FIREBASE_PROJECT_ID}")
